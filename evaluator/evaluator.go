...
func evalInfixExpression(operator string, left, right object.Object) object.Object {
	...
	// Down here we are performing pointer comparison, this is ok because there's only two instances of Boolean (the
...
func evalSetLiteral(node *ast.SetLiteral, environment *object.Environment) object.Object {
	pairs := make(map[object.HashKey]object.HashPair)

	for _, elementNode := range node.Elements {
		element := Eval(elementNode, environment)
		if isError(element) {
			return element
		}

		hashKey, ok := element.(object.Hashable)
		if !ok {
			return newError("unusable as hash key: %s", element.Type())
		}

		hashed := hashKey.HashKey()
		pairs[hashed] = object.HashPair{Key: element, Value: nil}
	}

	return &object.Set{Data: pairs}
}
