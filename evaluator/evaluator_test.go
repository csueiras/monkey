...
func TestSetLiteral(t *testing.T) {
	input := `let mySet = (1, 2, 3, 3, 2, 1); mySet;` // test set property by adding duplicates

	evaluated := testEval(input)
	result, ok := evaluated.(*object.Set)
	if !ok {
		t.Fatalf("object is not Set, got=%T (%+v)", evaluated, evaluated)
	}
	...
	sort.Slice(elements, func(i, j int) bool {
		left := elements[i].(*object.Integer)
		right := elements[j].(*object.Integer)
		return left.Value < right.Value
	})

	testIntegerObject(t, elements[0], 1)
	testIntegerObject(t, elements[1], 2)
	testIntegerObject(t, elements[2], 3)
}
