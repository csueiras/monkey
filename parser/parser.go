...
// really nice idiom for creating constants
...
func (p *Parser) parseLetStatement() *ast.LetStatement {
	...
	// We expect an identifier (variable name)
	...
	// Semicolons are optional so we only advance to the next token if current token is a semicolon
...
func (p *Parser) parseExpressionList(end token.TokenType) []ast.Expression {
	...
	for p.peekTokenIs(token.COMMA) {
		...
		p.nextToken() // go to start of element
		list = append(list, p.parseExpression(LOWEST))
	}
...
func (p *Parser) parseHashLiteral() ast.Expression {
	...
	for !p.peekTokenIs(token.RBRACE) {
		...
		if !p.peekTokenIs(token.RBRACE) && !p.expectPeek(token.COMMA) {
			// If we aren't closing the hash, or adding another element (expect a comma) then parse error
			return nil
		}
	}
...
func (p *Parser) parseSetLiteral() ast.Expression {
	set := &ast.SetLiteral{Token: p.curToken}

	if !p.expectPeek(token.LPAREN) {
		return nil
	}

	set.Elements = p.parseExpressionList(token.RPAREN)
	return set
}
